...
function getAll(location, parent){
    if(location.include === undefined){
        return _.omit(extend(parent, location), ['include']);
    }else{
        var all = Array();
        _.forIn(location.include, function(value, key){
            all = _.concat(all, getAll(value, extend(parent, location)));
        });
        return all;
    }
}
    ...
    if(names.length === 0){
        if(location === undefined && !_.isEmpty(parent)){
            ...
            _.forIn(location, function(value, key){
                all = _.concat(all, getAll(value, parent));
            });
            return all;
        }
    }else{
        ...
        if(location === undefined || location[name] === undefined){
            ...
            return lookInto(names.slice(1), location[name].include, extend(parent, location[name]));
        }
    }
}
